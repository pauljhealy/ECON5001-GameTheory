...
\bsk
\begin{itemize}
    \item $f(s)$: the outcome that strategy profile $s$ produces
    \item $U_i(\cdot)$: how much $i$ likes that outcome
    \item $\pi_i(s)=U_i(f(s))$: the two composed (utility \textit{as a function of strategies})
\end{itemize}
\vfill
...
Fix one player. Compare two of \textit{their own} strategies, $a$ and $b$.
\bsk
Ask: how do $a$ and $b$ compare \textbf{in every contingency they might face}?
\bsk
\begin{itemize}
...
\end{itemize}
\vfill
``contingency'' $=$ a choice of strategies by \textit{everyone else}.
\vspace{-0.4in}
\end{frame}
...
\end{itemize}
\bsk
Pick any $s_{-i}$. Then $\pi_i(a,s_{-i})>\pi_i(b,s_{-i})>\pi_i(a,s_{-i})$
\vfill
That would be a violation of transitivity, but payoffs/utilities (numbers) can't violate transitivity!!
\vspace{0.7in}
\end{frame}
