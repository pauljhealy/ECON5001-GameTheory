...
% Title page info
\title[Math Review]{Math Review (Not in the book)}
\author[ECON 5001]{ECON 5001\\Prof. Paul J. Healy} \color{metrop}
\institute[OSU]{}
...
\begin{frame}{Sets}
    ...
    \begin{itemize}
        ...
        \item No repetition (this isn't so important. Repetition $\Rightarrow$ ``multiset'')
    \end{itemize}
    \vfill
    \begin{itemize}
        \item ``Singleton'' set:
...
\begin{frame}{Math Language: Quantifiers}
    \begin{itemize}
        \item ``For all'' (or ``for every''): $\forall$
        \item ``There exists \uline{\ \ \ \ } such that \uline{\ \ \ \ }'': $\exists$ \uline{\ \ \ \ } s.t. \uline{\ \ \ \ }
    \end{itemize}
    \bsk
    \textbf{Examples:} Suppose $A=\{1,3\}$, $B=\{-1,2,4\}$ 
    \begin{itemize}
        \item<2->Is this true: $\forall x\in A\ \ \exists y\in B\ \ s.t.\ \ \ \ y>x$\\
        When $x=1$ set $y=2$. When $x=3$ set $y=4$\ $\checkmark$
        \item<3->Is this true: $\exists x\in B\ \ s.t.\ \ \forall y\in A\ \ \ \ x>y$\\
        Set $x=4$. When $y=1$ then $x>y$, and when $y=3$ then $x>y$\ $\checkmark$
        \item<4->Is this true: $\forall x\in B\ \ \exists y\in A\ \ s.t.\ \ \ \ y>x$\\
        What do you think?
    \end{itemize}
\end{frame}
...
A \textbf{partition} of a set $S$ carves it up with no gaps and no overlaps.
\bsk
\centering
\begin{tikzpicture}[scale=0.85]
  \draw[rounded corners=10pt, line width=1pt] (0,0) rectangle (9,2.6);
  \draw[line width=1pt] (2.4,0) -- (2.4,2.6);
  \draw[line width=1pt] (5.2,0) -- (5.2,2.6);
  \draw[line width=1pt] (5.2,1.3) -- (9,1.3);
  \node at (1.2,1.3) {$A_1$};
  \node at (3.8,1.3) {$A_2$};
  \node at (7.1,1.95) {$A_3$};
  \node at (7.1,0.65) {$A_4$};
  \node[above=2pt] at (4.5,2.6) {$S$};
\end{tikzpicture}
\bsk
\raggedright
Each $A_j$ is a \textbf{cell}. The whole collection $\{A_1,A_2,A_3,A_4\}$ is the
\textbf{partition}. (A set of sets!)
\vspace{-0.3in}
\end{frame}

\begin{frame}{Partitions}
Three requirements. In words, then in symbols:
\bsk
\begin{itemize}
    \item every cell is \textbf{nonempty}: \hspace{0.3in} $A_j\ne\emptyset$
    \bsk
    \item cells are \textbf{mutually exclusive}, no two overlap:
    $$\underbrace{A_j\cap A_k=\emptyset}_{\text{no element is in two cells}}
      \quad\text{for all } j\ne k$$
    \item cells \textbf{cover} $S$, together they use up everything:
    $$\underbrace{A_1\cup A_2\cup\cdots\cup A_m}_{\text{everything in some cell}}=S$$
\end{itemize}
\vfill
So every element of $S$ is in \textbf{exactly one} cell. \vspace{0.4in}
\end{frame}

\begin{frame}{Implies, and If and Only If}
$P\Rightarrow Q$ \ reads ``$P$ implies $Q$'' or ``if $P$, then $Q$''
\begin{itemize}
    \item whenever $P$ is true, $Q$ must be true
    \item it says \textbf{nothing} about what happens when $P$ is false
\end{itemize}
\bsk
$P\Leftrightarrow Q$ \ reads ``$P$ if and only if $Q$''
\begin{itemize}
    \item this is $P\Rightarrow Q$ \textit{and} $Q\Rightarrow P$, both directions
    \item $P$ and $Q$ are true in exactly the same situations
\end{itemize}
\vfill
\end{frame}

\begin{frame}{Implies, and If and Only If}
$$x=2 \quad\Longrightarrow\quad x^2=4$$
\bsk
True. But the arrow does not reverse: \vspace{0.5in}
\bsk
$$x^2=4 \quad\Longleftrightarrow\quad (x=2 \ \text{ or } \ x=-2)$$
\bsk
True in both directions. \vspace{0.5in}
\vfill
\end{frame}
...
Set $B$ & $p$ & \onslide<1>{$(r,p)$} & \onslide<1>{$(p,p)$} & \onslide<1>{$(c,p)$}    \\
    ...
    & \multicolumn{1}{c}{ }  & \multicolumn{1}{c}{$r$} & \multicolumn{1}{c}{$p$} & \multicolumn{1}{c}{$c$} \\
    & \multicolumn{1}{c}{ }  & \multicolumn{3}{c}{Set $A$} \\
\end{tabular}}\only<2->{\begin{tikzpicture}[>=stealth]
  % axes
...
List $(r,c)$ means P1 played Rock and P2 played Scissors. Order matters!!
\bsk
Sets vs. Lists:
\begin{center}
\begin{tabular}{c|c|c|}
...
\end{tabular}
\end{center}
Confusing: $(0,10)$ can represent a list or an interval. Context should clarify.
\end{frame}
...
\begin{itemize}
    ...
    \end{itemize}
    \item The entire profile can be written as $s=(s_i,s_{-i})$ for any arbitrary $i$
    \begin{itemize}
...
\begin{frame}{Sets Can Be Functions}
``\textbf{Set-builder notation}:''
\begin{itemize}
    \item Suppose $X = \{1,3,5,7,9\}$. Define:
    $$
        A = \{x\in X: x<6\}
    $$
    \item Read as: ``$x$ in $X$ such that $x<6$''
    \item Thus, $A=\{1,3,5\}$
\end{itemize}
\bsk
Sets can be functions of some parameter
\begin{itemize}
    \item Again, suppose $X=\{1,3,5,7,9\}$. Define the set function:
    $$
        A(b) = \{x\in X : x < b\}
    $$
    \item Here, $b$ can vary (it is a \textit{parameter} or \textit{input})
    \item $A(20) =$ \hspace{2in} $A(8) = $
    \item $A(2) =\ \  $ \hspace{2in} $A(0) = $    
\end{itemize}
\end{frame}

\begin{frame}{Game Theory Examples}
\begin{itemize}
    \item $\pi_i(s_i,s_{-i})$ will denote the payoff to player $i$ at strategy profile $s=(s_i,s_{-i})$\\
    \begin{matrixgame}[player names={Player 1,Player 2}, top={U, D}, left={L, R}]
        \payoffmatrix{3,4 & 0,5 \\ 5,0 & 0,2}
    \end{matrixgame}\\
    $\pi_1(U,L) = 3$,\ \ $\pi_1(U,R)=0$,\ \ $\pi_2(D,R)=2$, etc.
    \bsk
    \item Set of best replies for player $i$ (payoff maximizers) against strategy profile $s_{-i}$ is\\
    $$
        BR_i(s_{-i}) = \{s_i\in S_i : \pi_i(s_i,s_{-i}) \geq \pi_i(a,s_{-i})\ \forall a\in S_i \}
    $$
    \bsk
    \bsk
    \item $BR_1(L)=\{D\}$ \hspace{2in} $BR_1(R)=\{U,D\}$
...
\begin{frame}{A Bit of Calculus}
    ...
    \end{itemize}
      ...
      \draw[blue, thick, smooth, samples=100, domain=0:5]    plot (\x, {-15+12*(\x)-2*(\x)*(\x)});
    \end{tikzpicture}
\end{frame}

\begin{frame}{The End!}
    \vfill
    \large{That's enough for now!}
    \vfill
    \textbf{Please} ask questions if ever some math notation is unclear.
    \vfill
\end{frame}
